...
def draw_board(screen, game, BOARD_SIZE, SQUARE_SIZE, WHITE, CHESS_GREEN, REALGRAY, DOT_COLOR):
    ...
    if game.upgrade_mode:
        ...
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                ...
                if piece and piece.color == current_color and piece.piece_type == "pawn":
    ...
    if game.firing_mode:
        ...
        if game.selected_turret:
            ...
            for row in range(BOARD_SIZE):
                for col in range(BOARD_SIZE):
                    piece = game.board.get_piece_at(row, col)
                    if piece and piece.color == current_color and piece.piece_type == 'turret':
...
def draw_right_panel(screen, game, WIDTH, HEIGHT, RIGHT_PANEL_WIDTH, 
    ...
    if game.game_over:
        ...
        if play_again_button.collidepoint(mouse_pos):
            ...
            screen.blit(hover_surface, (WIDTH + 20, 180))
        ...
        swap_colors_color = (0, 0, 255)
        pygame.draw.rect(screen, swap_colors_color, swap_colors_button)
        
        if swap_colors_button.collidepoint(mouse_pos):
            hover_surface = pygame.Surface((160, 40))
            hover_surface.set_alpha(100)
            hover_surface.fill(HOVER_COLOR)
            screen.blit(hover_surface, (WIDTH + 20, 230))
    ...
    # "Who's turn is it?"
    turn_background_color = (128, 128, 128)
    turn_text_color = WHITE if game.turn.lower().startswith("white") else BLACK
    turn_text = font.render(f"Turn: {game.turn.replace('_', ' ').capitalize()}", True, turn_text_color)
    
    # Draw background rectangle
    turn_rect = pygame.Rect(WIDTH + 20, 30, turn_text.get_width() + 10, turn_text.get_height() + 5)
    pygame.draw.rect(screen, turn_background_color, turn_rect)
    
    # Draw text
    screen.blit(turn_text, (turn_rect.x + 5, turn_rect.y + 2))
    ...
    if not game.initial_phase:
        ...
        if has_turret:
            for row in range(8):
                for col in range(8):
                    piece = game.board.get_piece_at(row, col)
                    if piece and piece.color == piece_color and piece.piece_type == 'turret':
                        valid_targets = game.find_valid_targets_for_turret(row, col)
                        if valid_targets:
                            has_valid_targets = True
                            break
...
def draw_pieces(screen, game, BOARD_SIZE, SQUARE_SIZE, piece_images):
    for row in range(BOARD_SIZE):
        for col in range(BOARD_SIZE):
            piece = game.board.get_piece_at(row, col)
            if piece:
...
def handle_mouse_button_down(event, buttons, game, SQUARE_SIZE):
    if game.game_over:
